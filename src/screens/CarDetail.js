...
import { bookingSeat } from '../services/restclient/RestApi';
import { KeyboardAwareScrollView } from 'react-native-keyboard-aware-scroll-view';
import Toast from 'react-native-toast-message';

const CarDetail = ({ route, navigation }) => {
  const { val, Block, date, endDate } = route.params;
  console.log('datedatedatedatedatedatedatedatedate', date);
  const [email, setEmail] = useState('');
  const [enabled, setEnabled] = useState(false);
  const [loading, setLoading] = useState(false);
  const BookingSeatFunc = async () => {
    setLoading(true);
    setEnabled(true);
    if (email !== '') {
      bookingSeat(Block, val, email, date, endDate)
        .then((res) => {
          setLoading(false);
          setEnabled(false);
          const result = JSON.stringify(res);
          const resss = JSON.parse(result);
          console.log('dhbaysgiudhbhdsvfabuosbjdiuajs', resss);
